...
public class DocServiceImpl implements DocService {
    ...
    public List<DocumentChangeDTO> getDocChanges(Long id) {
        ...
        if (crdt == null) {
            // Transient, unshared instance rebuilt from the persisted blob — no locking needed.
            return documentChangeMapper.toDto(new Crdt(doc.getContent()).getItems());
        }
        // Read the live document under its monitor so we never traverse a half-applied edit.
        synchronized (crdt) {
    ...
    private void authorizeAccess(Doc doc) {
        String username = SecurityUtil.getCurrentUsername();
        if (!docAuthorizationService.canAccess(username, doc)) {
            throw new UnauthorizedUserException("You are not authorized to access this document");
        }
    }

    private void validatePermission(UserDocDTO userDocDTO) {
        Permission permission = userDocDTO.getPermission();
        if (permission == Permission.OWNER) {
            throw new IllegalArgumentException("Cannot assign OWNER permission. Only VIEW or EDIT is allowed.");
        }
        if (permission != Permission.VIEW && permission != Permission.EDIT) {
            throw new IllegalArgumentException("Invalid permission value. It should be VIEW or EDIT.");
        }
    }
}
