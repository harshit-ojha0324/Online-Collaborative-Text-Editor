...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<String> handleUnauthorizedUserException(UnauthorizedUserException unauthorizedUserException){
        // Authenticated but not permitted on this resource -> 403, not 401 (which means "not logged in").
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(unauthorizedUserException.getMessage());
    }
